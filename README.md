...
- **Editor configuration** – VS Code's C/C++ IntelliSense engine is disabled to avoid conflicting diagnostics. Install the [clangd extension](https://marketplace.visualstudio.com/items?itemName=llvm-vs-code-extensions.vscode-clangd) (included in `.vscode/extensions.json` recommendations) to enable language server features. The `.vscode/settings.json` configures clangd to use the `compile_commands.json` generated by CMake in `build/debug`, providing code completion, navigation, and inline diagnostics powered by clang-tidy.
  ```bash
